...
public class RayTrace implements IBlockReader {
  ...
  public LivingEntity getEntityInCrosshair(float partialTicks, double reachDistance) {
    ...
    Vec3d position = viewer.getEyePosition(partialTicks);
  ...
  private RayTraceContext setupRayTraceContext(PlayerEntity player, double distance,
    ...
    Vec3d fromPos = player.getEyePosition(1.0F);
    ...
    Vec3d toPos = fromPos.add((double) xComponent * distance, (double) yComponent * distance,
  ...
  public BlockRayTraceResult rayTraceBlocks(RayTraceContext context) {
    return IBlockReader.func_217300_a(context, (c, pos) -> {
      ...
      Vec3d v = c.func_222253_b().subtract(c.func_222250_a());
      return BlockRayTraceResult.createMiss(c.func_222250_a(),
          Direction.getFacingFromVector(v.x, v.y, v.z), new BlockPos(c.func_222250_a()));
    });
  }
}
